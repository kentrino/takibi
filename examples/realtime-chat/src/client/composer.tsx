...
import { MESSAGE_BODY_MAX_LENGTH, normalizeMessageBody, shouldClearComposer } from "../shared.ts";
...
export function Composer(props: {
  ...
  sending: boolean;
  sendError: string;
  onSend: (body: string) => Promise<"ok" | "error" | "stale">;
  onDraftChange: () => void;
}) {
  const { person, sending, sendError, onSend, onDraftChange } = props;
  ...
  async function submit(event: Event): Promise<void> {
    ...
    if (outcome === "ok" && shouldClearComposer(draftRef.current, submittedDraft)) {
      setDraft("");
      bodyRef.current?.focus();
    }
  }
