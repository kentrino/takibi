...
const allowedWorkspace = new Set(["@takibi/api", "@takibi/storage", "@takibi/worker-runtime"]);
const forbiddenSpecifiers = ["takibi", "@takibi/client", "@takibi/execution-model"];
...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      if (allowedWorkspace.has(workspace.name) || workspace.name === "@takibi/testing") {
        try {
...
test("testing package keeps a one-way dependency graph", () => {
  ...
  const workerRuntime = readManifest(join(packagesDir, "worker-runtime/package.json"));

  expect(testing.name).toBe("@takibi/testing");
  ...
  expect(testing.dependencies).toEqual({
    ...
    "@takibi/worker-runtime": "workspace:^",
  });
  ...
  expect(testing.dependencies?.["@takibi/execution-model"]).toBeUndefined();
...
test("no extracted package has a production dependency on Takibi or execution-model", () => {
  for (const directory of extractedPackages) {
    ...
    expect(manifest.dependencies?.["takibi"]).toBeUndefined();
    expect(manifest.dependencies?.["@takibi/execution-model"]).toBeUndefined();
    expect(manifest.devDependencies?.["@takibi/execution-model"]).toBeUndefined();
  }
});
...
test("testing source consumes the runtime bridge and owns node:sqlite", () => {
  ...
  expect(reachable.has(resolveWorkspaceEntry("@takibi/worker-runtime"))).toBe(true);
  expect(reachable.has(resolveWorkspaceEntry("@takibi/worker-runtime", "./testing-bridge"))).toBe(
    true,
  );
...
test("node:sqlite stays off Takibi production entries and worker-runtime", () => {
  ...
  const runtimeSrc = join(packagesDir, "worker-runtime/src");
  const clientSrc = join(packagesDir, "client/src");
  const entries = [
  ...
  for (const entry of entries) {
    ...
    expect(reachable.has("@takibi/testing"), entry).toBe(false);
  }
});
...
test("published testing declarations stay off Takibi and execution-model", () => {
  ...
  expect(js).toContain("@takibi/worker-runtime/testing-bridge");
