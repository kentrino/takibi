...
const allowedWorkspace = new Set(["@takibi/api", "@takibi/shared-types", "@takibi/storage"]);
...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/snapshot") {
        try {
...
test("snapshot package keeps a one-way dependency graph", () => {
  ...
  expect(snapshot.name).toBe("@takibi/snapshot");
