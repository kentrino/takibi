...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/takibi-snapshot") {
        try {
...
test("snapshot package keeps a one-way dependency graph", () => {
  const snapshot = readManifest(join(packageDir, "package.json"));
  ...
  expect(snapshot.name).toBe("@takibi/takibi-snapshot");
  expect(snapshot.exports?.["."]).toBe("./src/index.ts");
  expect(snapshot.files).toEqual(["dist", "README.md", "LICENSE"]);
  expect(snapshot.publishConfig?.exports).toMatchObject({
...
test("snapshot source stays off higher layers and Node or Worker runtime modules", () => {
...
test("published snapshot declarations do not import higher-layer or runtime modules", () => {
  ...
  expect(dts).not.toMatch(/src\/snapshot\.ts|src\/maintenance\.ts/);
  ...
  expect(dts).not.toContain("@takibi/takibi-policy");
  expect(js).not.toContain("@takibi/takibi-policy");
});
