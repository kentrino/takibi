...
const allowedWorkspace = new Set(["@takibi/policy", "@takibi/query", "@takibi/shared-types"]);
...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/api") {
        try {
...
test("api package keeps a one-way dependency graph", () => {
  ...
  expect(api.name).toBe("@takibi/api");
