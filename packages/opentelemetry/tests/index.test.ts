...
import { z } from "zod";
import { createOtelTakibiTracer, TakibiInstrumentation } from "@takibi/opentelemetry";
...
test("runtime boundary contains only the OTel API and Takibi instrumentation", () => {
  ...
  expect(runtimeImports).toEqual(["@opentelemetry/api", "takibi/instrumentation"]);
});

test("package README documents setup and isolate-local flushing", () => {
  const readme = readFileSync(join(import.meta.dirname, "../README.md"), "utf8");
  const source = readFileSync(join(import.meta.dirname, "../src/index.ts"), "utf8");
  expect(readme).toContain('from "@takibi/opentelemetry"');
  ...
  expect(readme).toContain("TAKIBI_ATTR");
  expect(readme).toContain("takibi/instrumentation");
  ...
  expect(readme).toContain("pnpm add takibi @takibi/opentelemetry @opentelemetry/api");
