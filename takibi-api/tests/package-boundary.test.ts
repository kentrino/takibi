...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/takibi-api") {
        try {
...
test("api package keeps a one-way dependency graph", () => {
  const api = readManifest(join(packageDir, "package.json"));
  ...
  expect(api.name).toBe("@takibi/takibi-api");
  expect(api.exports?.["."]).toBe("./src/index.ts");
  expect(api.files).toEqual(["dist", "README.md", "LICENSE"]);
  expect(api.publishConfig?.exports).toMatchObject({
...
test("api source and workspace exports stay browser-safe", () => {
  ...
  expect(reachable.has(resolveWorkspaceEntry("@takibi/takibi-query"))).toBe(true);
...
test("published api declarations do not reference private or higher-layer paths", () => {
  ...
  expect(dts).not.toMatch(/src\/action\.ts|src\/types\.ts|src\/errors\.ts/);
