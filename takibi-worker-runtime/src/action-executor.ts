...
import { actionExecutionPlan } from "./transaction-boundary";
import type { TakibiActionWork } from "./invocation-type-map";

export type { ActionInvocation } from "./action-gate";
export { resolveGateGrant };
...
export async function executeAction<TCtx extends object>(
  ...
  const plan = actionExecutionPlan(work);
  const handlerOf = (definition: RuntimeActionDefinition) =>
    createActionHandler({
      logger,
      definition,
  ...
  const prepareOn = async (scopedStorage: StorageDriver) => {
  ...
  return executePlan({
    ...
    none: {
      prepare: (_work, scopedStorage) => prepareOn(scopedStorage),
      apply: (resolved, scopedStorage) =>
        executeResolvedAction(
          { ...resolved, storage: scopedStorage },
          handlerOf(resolved.definition),
        ),
    },
    apply: {
      prepare: (_work, scopedStorage) => prepareOn(scopedStorage),
    ...
    full: {
      prepareAndApply: async (_work, scopedStorage) => {
        const resolved = await prepareOn(scopedStorage);
        return executeResolvedAction(resolved, handlerOf(resolved.definition), true);
      },
    },
  });
}
