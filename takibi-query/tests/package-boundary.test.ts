...
const allowedWorkspace = new Set(["@takibi/takibi-protocol", "@takibi/takibi-shared-types"]);
...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/takibi-query") {
        try {
...
test("query package keeps a one-way dependency graph", () => {
  ...
  expect(query.name).toBe("@takibi/takibi-query");
