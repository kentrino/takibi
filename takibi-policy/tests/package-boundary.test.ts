...
function walkGraph(entryFile: string): Set<string> {
  ...
  while (queue.length > 0) {
    ...
    for (const specifier of collectSpecifiers(readFileSync(file, "utf8"))) {
      ...
      visited.add(specifier);
      if (allowedWorkspace.has(specifier) || specifier === "@takibi/takibi-policy") {
        try {
...
test("policy package keeps a one-way dependency graph", () => {
  const policy = readManifest(join(packageDir, "package.json"));
  const sharedTypes = readManifest(join(packagesDir, "takibi-shared-types/package.json"));

  expect(policy.name).toBe("@takibi/takibi-policy");
  expect(policy.exports?.["."]).toBe("./src/index.ts");
  expect(policy.files).toEqual(["dist", "README.md", "LICENSE"]);
  expect(policy.publishConfig?.exports).toMatchObject({
...
test("policy source and workspace exports stay browser-safe", () => {
...
test("published policy declarations do not reference private or higher-layer paths", () => {
  ...
  expect(dts).not.toMatch(/src\/policy\.ts|src\/types\.ts/);
